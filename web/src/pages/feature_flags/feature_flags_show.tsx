...
export function FeatureFlagsShow() {
  const { orgId = '', id = '' } = useParams()
  const [item, setItem] = useState<FeatureFlag | null>(null)
  ...
  async function refresh() {
    setItem(await getFeatureFlag(id))
  }
  ...
  async function persistOverrides(next: FeatureFlagOverride[]) {
    setSaving(true)
    setError(null)
    try {
      setItem(await setFeatureFlagOverrides(id, { overrides: next }))
    } catch (e) {
  ...
  return (
    <section>
      ...
      <p className="row" style={{ gap: '0.75rem' }}>
        <Link className="button ghost" to={resourcePath(orgId, 'feature-flags', item.id, 'edit')}>
          Edit
        </Link>
        <Link className="button ghost" to={resourcePath(orgId, 'feature-flags')}>
          Back
        </Link>
      </p>
        ...
        <ul>
          {item.overrides.map((o) => (
            <li key={o.subject_id} className="row" style={{ gap: '0.75rem', marginBottom: '0.35rem' }}>
              ...
              <span>{o.effect}</span>
              <button type="button" className="button ghost" disabled={saving} onClick={() => void onRemoveOverride(o.subject_id)}>
                Remove
              </button>
            </li>
          ))}
