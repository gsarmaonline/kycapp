 ...
 *
 * `truncated` is the field that matters. A capped type reads as a complete one
 * unless the picture says otherwise, and a map that silently shows a hundred of
 * three thousand projects is worse than one that shows none.
 */
export type MerchantInstanceType = {
  type: string
 ...
 *
 * Separate from the schema because the schema is generated by core/reach, which
 * draws types on purpose. This is the application's own answer to "and which of
 * them do I have?".
 */
export function getMerchantInstances(orgId: string) {
  return request<MerchantInstances>(`/v1/organisations/${orgId}/access-instances`)
}
