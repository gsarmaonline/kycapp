...
Related: [authentication](authentication.md) · [authorisation](authorisation.md) · [data model](data-model.md) · [flows](flows.md) · [variable referencing](variables.md) · [Operator OpenAPI](openapi.yaml) · Integration OpenAPI (in-app **Documentation → Integration API**)
...
- `GET /v1/organisations/{id}/automations/catalog` — requires `automations:read`  
  Returns `triggers` (each may include `params` bindings), `actions` (+ params), `ops`, `condition_fields`, connected `databases` (for `db_insert`), connected `webhooks` (for `call_webhook`), `inbound_webhooks`, platform `plans`, org `roles`, and `schedule_presets` (cron shortcuts for `schedule.cron`).
