...
Triggers can also carry **`trigger_params`** (same idea as action `params`). Today used by `webhook.received`:

```json
{
  "name": "Partner hook",
  "trigger": "webhook.received",
  "trigger_params": { "inbound_webhook_id": "<hook_id>" },
  "conditions": { "all": [] },
  "actions": [
    { "type": "call_webhook", "params": { "webhook_id": "<outbound_id>" } }
  ]
}
```
...
| `databases` / `webhooks` / `inbound_webhooks` | Connected DBs, outbound hooks, and inbound endpoints for pickers |
